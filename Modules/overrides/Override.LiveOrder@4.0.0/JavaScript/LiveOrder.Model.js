	...
	)
{
	...
	,	wrapOptionsSuccess: function (options)
		...
		// @method addItem @param {Item.Model} item @param {Object} options @return {jQuery.Deferred}
	,	addItem: function (item, options)
		...
		// @method addItem @param {Array<Item.Model>} item @param {Object} options @return {jQuery.Deferred}
	,	addItems: function (items, options)
		{
			...
			var lines = _.map(items, function (item)
			{
                ...
                ,   image = _.first(item.get('_images'))
                ,   imageURL = image.url
                ...
                //console.log(item.get('_images'), imageURL, item);
                
                // set the item thumbnail value
                // don't pass if its somehow set to an image not available
                line_options['custcol_sc_item_image'] = (imageURL.indexOf('no_image_available') > -1) ? '' : imageURL;
                line_options['custcol_sc_item_title'] = itemTitle;
                line_options['custcol_sc_item_sku'] = itemSku;
                line_options['custcol_web_free_ship'] = custitem_web_free_ship ? 'T' : 'F';
		...
		// @method updateItem @param {String} line_id @param {Item.Model} item @param {Object} options @return {jQuery.Deferred}
