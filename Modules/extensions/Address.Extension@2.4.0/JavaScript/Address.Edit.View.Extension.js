			...
			,   validateAddresses: function () {
						...
						.done(function (data) {
						    ...
						    try {
								...
								if (data && data.errorCode) {
									...
									$('[data-type="alert-placeholder"]:first').html(
				...
				function buildDomElements(index, addressesArr)
				{
					...
					if (addressesArr.length === 1) {
						...
						validationColumn.className += 'address-validation-column-missing';
					...
					for (var i = 0; i < addressesArr.length; i++) {
						var validationColumn = document.createElement('div');
						validationColumn.className += 'address-validation-column';
						validationColumn.innerHTML = '<label class="' + (addressesArr[i].isOriginal ? 'original' : 'suggested') + '"><div class="address-validation-control">' + (addressesArr[i].isOriginal ? '' : '<span class="address-validation-suggested">SUGGESTED</span>')
							+ '<input type="radio" id="form' + index + 'Option' + i + '" class="address-validation-radio" name="validatedAddress" data-addr1="' + addressesArr[i].addr1 + '" data-addr2="' + addressesArr[i].addr2 + '" data-state="' + addressesArr[i].state + '" data-city="' + addressesArr[i].city + '" data-zip="' + addressesArr[i].zip + '" data-isresidential="' + addressesArr[i].isresidential + '" /></div><div class="address-validation-address"><p class="address-validation-text"><span>' + addressesArr[i].addr1 + '</span><span>' + addressesArr[i].addr2 + '</span><span>' + addressesArr[i].city + ', ' + addressesArr[i].state + ' ' + addressesArr[i].zip + '</span></p></div></label>';
						addressValidationRow.appendChild(validationColumn);
					}
